...
describe('StreamingState', () => {
  ...
  describe('stream aggregation compatibility', () => {
    ...
    it('does not render a signature-only thinking block after final text', () => {
      const processor = new PartProcessor(state);
      const signature = Buffer.from('final text signature', 'utf-8').toString('base64');

      const textChunks = processor.process({
        text: 'answer',
      ...
      const output = [...textChunks, ...finishChunks].join('');

      expect(output).toContain('"type":"text"');
      expect(output).toContain('"text":"answer"');
      expect(output).not.toContain('"type":"thinking"');
      expect(output).not.toContain('signature_delta');
    });

    it('drops empty thought signature tails in streaming output', () => {
      const processor = new PartProcessor(state);
      const signature = Buffer.from('empty thought signature', 'utf-8').toString('base64');

      const chunks = [
        ...processor.process({ text: 'answer' }),
        ...processor.process({ text: '', thought: true, thoughtSignature: signature }),
