...
export class GeminiClient {
  ...
  private createInternalRequestBody(path: string, body: GeminiInternalRequest): string | Readable {
    ...
    this.dumpInternalRequestBody(path, bodyText);
  ...
  private dumpInternalRequestBody(requestPath: string, bodyText: string): void {
    const captureDir = process.env.ANTIGRAVITY_CAPTURE_DIR?.trim();
    if (!captureDir) {
      return;
    }
    ...
    try {
      ...
      const parsed = JSON.parse(bodyText) as { model?: unknown; requestId?: unknown };
      const model = isString(parsed.model) ? parsed.model.replace(/[^a-zA-Z0-9_.-]/g, '_') : 'unknown';
      const suffix = requestPath.includes('streamGenerateContent') ? 'stream' : 'generate';
      const filename = `${Date.now()}-${suffix}-${model}.json`;
      fs.writeFileSync(path.join(captureDir, filename), JSON.stringify(parsed, null, 2), 'utf-8');
    } catch (error) {
      this.logger.warn(
        `Failed to dump Antigravity upstream request body: ${
