...
export class GeminiController {
  ...
  async listModels(@Res() res: FastifyReply) {
    const models = await this.buildGeminiModelList();
  ...
  @Get('models/:model')
  async getModel(@Param('model') model: string, @Res() res: FastifyReply) {
    const targetName = model.startsWith('models/') ? model : `models/${model}`;
    const matched = (await this.buildGeminiModelList()).find((item) => item.name === targetName);
  ...
  private async handleModelActionDispatch(
    ...
    try {
      if (action === 'streamGenerateContent') {
        const stream = await this.proxyService.handleGeminiStreamGenerateContent(
          model,
          body,
          this.proxyService.createCredentialScopeFromHeaders(
