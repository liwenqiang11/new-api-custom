...
const DEFAULT_SUBCLIENT_TYPE = 'hub';
const DEFAULT_REMOTE_TIMEOUT_MS = 2500;
const VERSION_REGEX = /\d+\.\d+\.\d+/g;

type UserAgentSource = 'env' | 'local' | 'remote' | 'changelog' | 'fallback';
...
export function buildUserAgent(version: string): string {
  const subclientType = normalizeNonEmptyString(process.env.ANTIGRAVITY_SUBCLIENT_TYPE) ?? DEFAULT_SUBCLIENT_TYPE;
  return `antigravity/${subclientType}/${version} ${getPlatformTag()}/${getArchTag()}`;
}
...
async function resolveDefaultUserAgentResolution(): Promise<UserAgentResolution> {
  ...
  const envVersion = normalizeNonEmptyString(process.env.ANTIGRAVITY_VERSION);
  if (envVersion) {
    return {
      source: 'env',
      version: envVersion,
      userAgent: buildUserAgent(envVersion),
    };
  }
