...
import { ProxyRequestCredentialScope, ProxyService } from './proxy.service';
...
export class ProxyController {
  ...
  async listModels(@Res() res: FastifyReply) {
    try {
      ...
      const candidateModelIds = getAllDynamicModels(
        customMapping,
        this.tokenManager?.getAllCollectedModels(),
      );
      const modelIds = this.tokenManager
        ? await this.tokenManager.getUsableModelIds(candidateModelIds)
        : candidateModelIds;
  ...
  @Post('internal/accounts/inspect')
  async inspectAccounts(
    @Body() body: { credentials?: Array<Record<string, unknown>> },
    ...
    try {
      ...
      if (credentials.length === 0) {
        res.status(HttpStatus.BAD_REQUEST).send({
          error: {
            message: 'Missing credentials payload',
      ...
      const data = await this.tokenManager?.inspectCredentials(credentials);
      res.status(HttpStatus.OK).send({
        object: 'list',
        data: data ?? [],
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Failed to inspect accounts';
      this.logger.error(message, error instanceof Error ? error.stack : undefined);
      res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({
        error: {
          message,
  ...
  async completions(
    ...
    try {
      const result = await this.proxyService.handleChatCompletions(
        request,
        this.credentialScopeFromRequest(req),
      );
      ...
      this.applyModelRoutingHeaders(res, result);
  ...
  async imageGenerations(
    ...
    await this.sendOpenAIImageGenerationResponse(request, body.prompt ?? '', res, req);
  }
  ...
  private async respondOpenAIChatCompletions(
    ...
    try {
      const result = await this.proxyService.handleChatCompletions(
        body,
        req ? this.credentialScopeFromRequest(req) : undefined,
      );
  ...
  private credentialScopeFromRequest(req: FastifyRequest): ProxyRequestCredentialScope | undefined {
    return this.proxyService.createCredentialScopeFromHeaders(
  ...
  private buildModelRoutingHeaders(result: unknown): Record<string, string> {
    const routing = this.proxyService.getModelRoutingInfo(result);
    if (!routing) {
      return {};
    }
    const headers: Record<string, string> = {
      'X-Antigravity-Requested-Model': routing.requestedModel,
      'X-Antigravity-Mapped-Model': routing.mappedModel,
      'X-Antigravity-Upstream-Model': routing.upstreamModel,
    };
    if (routing.reason) {
      headers['X-Antigravity-Model-Redirect-Reason'] = routing.reason;
    }
    return headers;
  }

  private applyModelRoutingHeaders(res: FastifyReply, result: unknown): void {
    for (const [key, value] of Object.entries(this.buildModelRoutingHeaders(result))) {
      res.header(key, value);
    }
  }

  private writeSseResponse(res: FastifyReply, stream: Observable<unknown>): void {
    const routingHeaders = this.buildModelRoutingHeaders(stream);
    if (
      !res.raw ||
      ...
      for (const [key, value] of Object.entries(routingHeaders)) {
        res.header(key, value);
      }
      res.send(stream);
      return;
