...
import { QuotaData, LoadProjectResponse, QuotaApiResponse, QuotaWindowInfo } from './types';
import { logger } from '@/shared/logging/logger';
import { resolveRequestUserAgent } from '../server/request-user-agent';
...
function extractQuotaWindows(modelInfo: QuotaApiResponse['models'][string]): {
  weekly?: QuotaWindowInfo;
  five_hour?: QuotaWindowInfo;
} {
  const quotaInfo = modelInfo.quotaInfo as (RawQuotaInfo & Record<string, unknown>) | undefined;
  const quotaInfos =
    modelInfo.quotaInfos ?? (quotaInfo?.quotaInfos as Record<string, RawQuotaInfo> | undefined);
  const weekly = firstQuotaWindow(
    modelInfo.weeklyQuotaInfo,
    modelInfo.weeklyQuota,
    modelInfo.quotaInfoWeekly,
    quotaInfos?.weekly,
    quotaInfos?.week,
  ...
  const fiveHour = firstQuotaWindow(
    modelInfo.fiveHourQuotaInfo,
    modelInfo.fiveHourQuota,
    modelInfo.quotaInfo5h,
    quotaInfos?.fiveHour,
...
export class QuotaService {
  private static async createClient(timeoutSecs: number = 15): Promise<AxiosInstance> {
    const userAgent = await resolveRequestUserAgent();
    return axios.create({
  ...
  private static toQuotaData(
    ...
    for (const [modelName, modelInfo] of Object.entries(quotaResponse.models)) {
      ...
      const quotaWindows = extractQuotaWindows(modelInfo);
