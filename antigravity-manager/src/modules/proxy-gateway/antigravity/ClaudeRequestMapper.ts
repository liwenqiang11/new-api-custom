...
export function transformClaudeRequestIn(
  ...
  const systemInstruction = mergeSystemInstructionParts(
    buildSystemInstruction(claudeReq.system),
    collectSystemMessageParts(claudeReq.messages),
  );
  ...
  if (isThinkingEnabled) {
    ...
    if (hasFunctionCalls && !hasValidSignatureForFunctionCalls(claudeReq.messages, globalSig)) {
      // A thought signature is stateful upstream data. Do not fabricate one for
      // a converted tool turn; send the request without thinking instead.
...
function shouldEnableThinkingByDefault(mappedModel: string, originalModel: string): boolean {
  ...
  if (mappedLower.includes('gemini-3.1-pro-high') || mappedLower.includes('gemini-3.1-pro-low')) {
    return false;
  }
  return (
...
function buildSystemInstruction(
  ...
  if (system) {
    if (isString(system)) {
      ...
      if (text && !isDownstreamHarnessText(text)) {
        parts.push({ text });
      }
    } else if (Array.isArray(system)) {
      for (const block of system) {
...
function collectSystemMessageParts(messages: Message[]): { text: string }[] {
  const parts: { text: string }[] = [];
  for (const message of messages) {
    ...
    for (const block of contentBlocks) {
      if (block.type === 'text' && block.text && !isEmpty(block.text.trim())) {
        const text = block.text.trim();
        if (!isDownstreamHarnessText(text)) {
          parts.push({ text });
        }
      }
    }
...
function mergeSystemInstructionParts(
  systemInstruction: { parts: { text: string }[] } | null,
  extraParts: { text: string }[],
): { parts: { text: string }[] } | null {
  ...
  return {
    parts: [...systemInstruction.parts, ...extraParts],
  };
}

function isDownstreamHarnessText(text: string): boolean {
  ...
  if (exactOrPrefixMarkers.some((marker) => lower.startsWith(marker))) {
    return true;
  }
  ...
  return containsMarkers.some((marker) => lower.includes(marker));
}
...
function normalizeThoughtSignatureForGemini(signature?: string | null): string | null {
  ...
  return normalizeExistingBase64(trimmed);
}

function normalizeExistingBase64(value: string): string | null {
  ...
  try {
    ...
    return canonical.replace(/=+$/, '') === padded.replace(/=+$/, '') ? canonical : null;
  } catch {
    return null;
  }
}
...
function hasValidSignatureForFunctionCalls(
  ...
  if (normalizeThoughtSignatureForGemini(globalSig)) {
    return true;
  }
      ...
      if (Array.isArray(msg.content)) {
        for (const block of msg.content) {
          if (
            block.type === 'thinking' &&
            block.signature &&
            normalizeThoughtSignatureForGemini(block.signature)
          ) {
            return true;
          }
        }
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    for (const block of contentBlocks) {
      if (block.type === 'text') {
        ...
        const signature = normalizeThoughtSignatureForGemini(block.signature);
        if (!signature && !isThinkingEnabled) {
          continue;
        }
        ...
        const finalSig =
          ...
          normalizeThoughtSignatureForGemini(SignatureStore.get());
        if (finalSig) {
...
function buildGenerationConfig(
  ...
  if (isOpenAIPath) {
    ...
    if (claudeReq.max_tokens !== undefined) {
      ...
      (config as GenerationConfig & { __explicitMaxOutputTokens?: boolean }).__explicitMaxOutputTokens =
        true;
    } else {
      config.maxOutputTokens = getMaxOutputTokens(mappedModel);
    }
    const stopSequences = filterClaudeCodeStopSequences(claudeReq.stop_sequences);
  ...
  const stopSequences = isThinkingEnabled
    ? []
    : filterClaudeCodeStopSequences(claudeReq.stop_sequences);
  if (stopSequences.length > 0) {
    config.stopSequences = stopSequences;
  }
  return config;
...
function filterClaudeCodeStopSequences(stopSequences?: string[]): string[] {
  if (!Array.isArray(stopSequences)) {
    return [];
  }

  const protocolSentinels = new Set([
    '<|user|>',
    '<|assistant|>',
    '<|system|>',
    '<|endoftext|>',
  ...
  return stopSequences
    .filter((item): item is string => isString(item) && !isEmpty(item))
    .filter((item) => !protocolSentinels.has(item));
}
