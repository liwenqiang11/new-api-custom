...
export class StreamingState {
  ...
  public emitFinish(finishReason?: string, usageMetadata?: UsageMetadata): string[] {
    ...
    // Drop a signature-only tail. Claude Code renders an empty thinking block as a
    // second "Thought for Ns" section after the answer, which is only UI noise here.
...
export class PartProcessor {
  ...
  private processText(text: string, signature?: string): string[] {
    ...
    // Empty text/signature-only parts are internal bookkeeping. Do not render them.
    if (!text) {
      return chunks;
    }
