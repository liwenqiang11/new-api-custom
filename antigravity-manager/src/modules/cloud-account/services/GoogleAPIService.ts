...
const QUOTA_SUMMARY_API_ENDPOINTS = [
...
export interface ModelQuotaInfo {
  percentage: number;
  resetTime: string;
  weekly?: QuotaWindowInfo;
  five_hour?: QuotaWindowInfo;
  quota_windows?: {
    weekly?: QuotaWindowInfo;
    five_hour?: QuotaWindowInfo;
  };
...
interface ModelInfoRaw {
  quotaInfo?: RawQuotaInfo & Record<string, unknown>;
  weeklyQuotaInfo?: RawQuotaInfo;
  weeklyQuota?: RawQuotaInfo;
  quotaInfoWeekly?: RawQuotaInfo;
  fiveHourQuotaInfo?: RawQuotaInfo;
  fiveHourQuota?: RawQuotaInfo;
  quotaInfo5h?: RawQuotaInfo;
  quotaInfos?: Record<string, RawQuotaInfo | undefined>;
  displayName?: string;
...
function isRawQuotaInfo(value: unknown): value is RawQuotaInfo {
  if (!value || typeof value !== 'object') {
    return false;
  }
  const quota = value as RawQuotaInfo;
  return quota.remainingFraction !== undefined || quota.resetTime !== undefined;
}

function normalizeQuotaWindow(value: unknown): QuotaWindowInfo | undefined {
  if (!isRawQuotaInfo(value)) {
    return undefined;
  }
  const fraction = Number(value.remainingFraction ?? 0);
  return {
    percentage: Math.floor((Number.isFinite(fraction) ? fraction : 0) * 100),
    resetTime: value.resetTime || '',
  };
}

function firstQuotaWindow(...values: unknown[]): QuotaWindowInfo | undefined {
  for (const value of values) {
    const window = normalizeQuotaWindow(value);
    if (window) {
      return window;
    }
  }
...
function normalizeSummaryBucket(bucket: QuotaSummaryBucketRaw | undefined): QuotaWindowInfo | undefined {
  if (!bucket) {
    return undefined;
  }
...
function isWeeklyQuotaBucket(bucket: QuotaSummaryBucketRaw): boolean {
  const text = `${bucket.window || ''} ${bucket.bucketId || ''} ${bucket.displayName || ''}`.toLowerCase();
  return text.includes('weekly') || text.includes('week');
}

function isFiveHourQuotaBucket(bucket: QuotaSummaryBucketRaw): boolean {
  const text = `${bucket.window || ''} ${bucket.bucketId || ''} ${bucket.displayName || ''}`.toLowerCase();
  return text.includes('5h') || text.includes('five hour') || text.includes('five-hour');
}

function quotaSummaryGroupMatchesModel(group: QuotaSummaryGroupRaw, modelName: string): boolean {
  const model = modelName.toLowerCase();
  const text = `${group.displayName || ''} ${group.description || ''} ${(group.buckets || [])
  ...
  if (model.includes('claude') || model.includes('gpt')) {
    return text.includes('claude') || text.includes('gpt') || text.includes('3p');
  }
  return false;
}

function mergeQuotaSummaryIntoQuotaData(quotaData: QuotaData, summary: QuotaSummaryResponse | null): void {
  if (!summary?.groups?.length) {
    return;
  }

  for (const group of summary.groups) {
    const weekly = normalizeSummaryBucket(group.buckets?.find(isWeeklyQuotaBucket));
    const fiveHour = normalizeSummaryBucket(group.buckets?.find(isFiveHourQuotaBucket));
    if (!weekly && !fiveHour) {
      continue;
    }

    for (const [modelName, modelQuota] of Object.entries(quotaData.models)) {
      if (!quotaSummaryGroupMatchesModel(group, modelName)) {
        continue;
      }

      if (weekly) {
        modelQuota.weekly = weekly;
      }
      if (fiveHour && !modelQuota.five_hour) {
        modelQuota.five_hour = fiveHour;
      }
      modelQuota.quota_windows = {
        weekly: modelQuota.weekly,
        five_hour: modelQuota.five_hour,
      };
    }
  }
}

function extractQuotaWindows(info: ModelInfoRaw): {
  weekly?: QuotaWindowInfo;
  five_hour?: QuotaWindowInfo;
} {
  const quotaInfo = info.quotaInfo as (RawQuotaInfo & Record<string, unknown>) | undefined;
  const quotaInfos = info.quotaInfos ?? (quotaInfo?.quotaInfos as Record<string, RawQuotaInfo> | undefined);
  const weekly = firstQuotaWindow(
    info.weeklyQuotaInfo,
    info.weeklyQuota,
    info.quotaInfoWeekly,
    quotaInfos?.weekly,
    quotaInfos?.week,
    quotaInfos?.weeklyQuota,
    quotaInfo?.weekly,
    quotaInfo?.weeklyQuota,
  );
  const fiveHour = firstQuotaWindow(
    info.fiveHourQuotaInfo,
    info.fiveHourQuota,
    info.quotaInfo5h,
    quotaInfos?.fiveHour,
    quotaInfos?.five_hour,
    quotaInfos?.['5h'],
    quotaInfo?.fiveHour,
    quotaInfo?.five_hour,
    quotaInfo?.['5h'],
...
function toModelQuotaInfo(modelName: string, info: ModelInfoRaw): ModelQuotaInfo | null {
  if (!isTrackedModel(modelName)) {
    return null;
  }

  const quotaWindows = extractQuotaWindows(info);
  const primaryWindow = quotaWindows.five_hour ?? quotaWindows.weekly;
  if (!primaryWindow) {
    return null;
  }
  ...
  return {
    ...
    five_hour: quotaWindows.five_hour,
...
export class GoogleAPIService {
  ...
  static async fetchQuotaSummary(
    accessToken: string,
    proxyUrl?: string,
    ...
    for (const endpoint of QUOTA_SUMMARY_API_ENDPOINTS) {
      try {
        const response = await fetch(endpoint, {
          method: 'POST',
          headers: buildInternalApiHeaders(accessToken),
        ...
        return (await response.json()) as QuotaSummaryResponse;
      } catch (error: unknown) {
    ...
    if (lastError) {
      logger.warn(
        `[GoogleAPIService] Failed to fetch quota summary: ${lastError.message}`,
      );
    }
    return null;
  }
    ...
    for (let endpointIndex = 0; endpointIndex < QUOTA_API_ENDPOINTS.length; endpointIndex++) {
      ...
      while (true) {
        try {
          ...
          const quotaSummary = await this.fetchQuotaSummary(accessToken, proxyUrl);
          mergeQuotaSummaryIntoQuotaData(result, quotaSummary);
