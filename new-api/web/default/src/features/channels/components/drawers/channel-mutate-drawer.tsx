...
export function ChannelMutateDrawer({
  ...
  const isCodexChannel = currentType === 57
  const isAntigravityChannel = currentType === 58
  const isOAuthJsonChannel = isCodexChannel || isAntigravityChannel
  ...
  const modelOptions = useMemo(() => {
    const baseModels =
      currentType === 58 ? [...ANTIGRAVITY_MODELS] : allModelsList
    const allModels = new Set([...baseModels, ...currentModelsArray])
                    ...
                    <ChannelAuthSection>
                      ...
                      <FormField
                        ...
                        render={({ field }) => {
                          const keyPlaceholder = (() => {
                            if (isEditing) {
                              return isOAuthJsonChannel
                                ? t(
                                    'Leave empty to keep existing OAuth JSON credential'
                                  )
                                : t('Leave empty to keep existing key')
                            }
                            if (isAntigravityChannel) {
                              return isBatchMode
                                ? t(
                                    'Paste a JSON array, or one Antigravity OAuth/credential-store JSON object per line'
                                  )
                                : t('Paste Antigravity OAuth or credential-store JSON')
                            }
                            if (isCodexChannel) {
                              return t('Paste Codex auth JSON, for example:\n{\n  "tokens": {\n    "id_token": "...",\n    "access_token": "...",\n    "refresh_token": "...",\n    "account_id": "..."\n  }\n}')
                            }
                            if (currentType === 33) {
                                  ...
                                  <span>
                                    {isEditing ? (
                                      <>
                                        {isOAuthJsonChannel
                                          ? t(
                                              'Enter a new OAuth JSON credential to update, or leave empty to keep the current credential'
                                            )
                                          : t(
                                        ...
                                        {isMultiKeyChannel && (
                                          <span className='text-warning mt-1 block'>
                                            {t(
                                              isOAuthJsonChannel
                                                ? 'Multi-credential channel: Credentials will be'
                                    ...
                                    ) : isAntigravityChannel && isBatchMode ? (
                                      t(
                                        'Paste a JSON array of Antigravity OAuth credentials, or one credential-store JSON object per line'
                                      )
                                    ) : isAntigravityChannel ? (
                                      t(
                                        'Paste Antigravity OAuth JSON, or the native credential-store JSON copied from Windows Credential Manager.'
                                      )
                                    ) : isCodexChannel ? (
                                      t(
                                        'Paste the Codex auth JSON generated by authorization. It must contain the nested tokens object.'
                                      )
                                    ) : isBatchMode ? (
                                      t(
                              ...
                              {isEditing && (
                                <div className='border-border/60 mt-4 flex flex-col gap-3 border-y border-dashed py-4'>
                                  <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                                    <div>
                                      ...
                                      <p className='text-muted-foreground text-xs'>
                                        {t(
                                          isOAuthJsonChannel
                                            ? 'Verification required to reveal the saved credential.'
                                  ...
                                  <Input
                                    ...
                                    placeholder={
                                      isOAuthJsonChannel
                                        ? t('Hidden credential - verify to reveal')
                                        : t('Hidden - verify to reveal')
                                    }
                                    className='font-mono'
                                  />
                                </div>
                          ...
                          <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                            <div className='flex flex-col gap-0.5'>
                              ...
                              <div className='text-muted-foreground text-xs'>
                                {t(
                                  'Codex channels use an auth JSON credential with nested tokens as the key.'
                                )}
                              </div>
                            </div>
                      ...
                      {isAntigravityChannel && (
                        <div className='border-border/60 flex flex-col gap-3 border-y py-4'>
                          <div className='flex flex-col gap-0.5'>
                            <div className='text-sm font-semibold'>
                              {t('Antigravity OAuth JSON')}
                            </div>
                            <div className='text-muted-foreground text-xs'>
                              {t(
                                'Antigravity channels use a Google OAuth JSON credential as the upstream account.'
                              )}
                            </div>
                          </div>
                          <Alert>
                            <AlertDescription>
                              {t(
                                'Paste OAuth JSON or native credential-store JSON into the credential field. access_token is required; refresh_token enables automatic refresh.'
                              )}
                            </AlertDescription>
                          </Alert>
                        </div>
