...
} from './antigravity-accounts-dialog'
import {
  CodexUsageDialog,
...
export function BalanceQueryDialog({
  ...
  const isAntigravity = currentRow?.type === 58
  ...
  const handleQueryAntigravityAccounts = async () => {
    const row = currentRow
    if (!row) return
    setIsQuerying(true)
    try {
      const res = await getAntigravityAccounts(row.id)
      if (!res.success) {
  ...
  useEffect(() => {
    ...
    if (!open) return
    handleQueryAntigravityAccounts()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, isAntigravity])
  ...
  const handleClose = () => {
    ...
    setAntigravityAccountsResponse(null)
    onOpenChange(false)
  }
  ...
  if (isAntigravity) {
    return (
      <AntigravityAccountsDialog
        ...
        channelName={currentRow.name}
        channelId={currentRow.id}
        configuredModels={currentRow.models?.split(',').map((m) => m.trim()).filter(Boolean)}
        response={antigravityAccountsResponse}
        onRefresh={handleQueryAntigravityAccounts}
        isRefreshing={isQuerying}
      />
    )
  }
