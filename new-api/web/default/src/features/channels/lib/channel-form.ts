...
function isCodexCredential(value: string | undefined): boolean {
  try {
    ...
    return (
      isJsonObjectValue(tokens) &&
      typeof tokens.id_token === 'string' &&
      tokens.id_token.trim().length > 0 &&
...
function isAntigravityCredentialObject(value: unknown): boolean {
  const token = isJsonObjectValue(value) ? value.token : undefined
  return (
    (isJsonObjectValue(value) &&
      typeof value.access_token === 'string' &&
      value.access_token.trim().length > 0) ||
    (isJsonObjectValue(token) &&
      typeof token.access_token === 'string' &&
      token.access_token.trim().length > 0)
  )
}

function isAntigravityCredential(value: string | undefined): boolean {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.length > 0 && parsed.every(isAntigravityCredentialObject)
    }
    return isAntigravityCredentialObject(parsed)
  } catch {
    return trimmed
      .split('\n')
      .map((line) => line.trim())
      ...
      .every((line) => {
        try {
          return isAntigravityCredentialObject(JSON.parse(line))
        } catch {
          return false
        }
      })
...
export const channelFormSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.type === 57) {
      ...
      if (data.key?.trim() && !isCodexCredential(data.key)) {
        addRequiredIssue(
          ctx,
          'key',
          'Codex credential must include tokens.id_token, tokens.access_token, tokens.refresh_token, and tokens.account_id'
        )
      }
    }
    ...
    if (
      data.type === 58 &&
      data.key?.trim() &&
      !isAntigravityCredential(data.key)
    ) {
      addRequiredIssue(
        ctx,
        'key',
        'Antigravity credential must be OAuth JSON with access_token, native credential-store JSON, a JSON array, or one JSON object per line'
      )
    }
