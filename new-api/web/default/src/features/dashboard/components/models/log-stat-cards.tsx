...
export function LogStatCards(props: LogStatCardsProps) {
  ...
  useEffect(() => {
    ...
    const timeRange = filters?.time_range_all
      ? {
          start_timestamp: 0,
    ...
    setTimeRangeMinutes(Math.max(timeDiff, 1))

    getUserQuotaDates(buildQueryParams(timeRange, filters), isAdmin)
      .then((res) => {
        if (abortController.signal.aborted) return
        const data = res?.data || []
        if (filters?.time_range_all && data.length > 0) {
          ...
          if (timestamps.length > 0) {
            ...
            setTimeRangeMinutes(Math.max((maxTimestamp - minTimestamp) / 60, 1))
          }
        }
