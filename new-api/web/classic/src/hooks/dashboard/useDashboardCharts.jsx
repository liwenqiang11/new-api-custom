...
export const useDashboardCharts = (
  ...
  const [spec_token_model_line, setSpecTokenModelLine] = useState({
    type: 'line',
    data: [
      {
        id: 'tokenLineData',
  ...
  const updateChartData = useCallback(
    (data) => {
      ...
      for (let [_, value] of aggregatedData) {
        ...
        updateMapValue(modelTokenTotals, value.model, value.tokens || 0);
      }
      ...
      const newTokenPieData = Array.from(modelTokenTotals)
        .map(([model, tokens]) => ({
          type: model,
          value: tokens,
        }))
      ...
      let newTokenLineData = [];

      chartTimePoints.forEach((time) => {
        let timeData = Array.from(uniqueModels).map((model) => {
          const key = `${time}-${model}`;
          const aggregated = aggregatedData.get(key);
          const rawTokens = aggregated?.tokens || 0;
          return {
            Time: time,
            Model: model,
            rawTokens,
        ...
        timeData = timeData.map((item) => ({ ...item, TimeSum: timeSum }));
        newTokenLineData.push(...timeData);
      });

      newTokenLineData.sort((a, b) => a.Time.localeCompare(b.Time));
      ...
      chartTimePoints.forEach((time) => {
        ...
        const tokenTimeData = Array.from(uniqueModels).map((model) => {
          const key = `${time}-${model}`;
          const aggregated = aggregatedData.get(key);
          return {
            Time: time,
        ...
        tokenModelLineData.push(...tokenTimeData);
      });
      modelLineData.sort((a, b) => a.Time.localeCompare(b.Time));
      tokenModelLineData.sort((a, b) => a.Time.localeCompare(b.Time));
  ...
  const updateUserChartData = useCallback(
    (data) => {
      ...
      const userTokenTrendValues = userTrend.map((item) => ({
        Time: item.Time,
        User: item.User,
        Tokens: item.Tokens || 0,
      }));

      const totalUserTokens = rankingData.reduce(
