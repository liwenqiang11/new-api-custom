...
const Dashboard = () => {
  ...
  // ========== 数据处理 ==========
  const loadUserData = async (rangeOverride = null) => {
    if (dashboardData.isAdminUser) {
      const userData = await dashboardData.loadUserQuotaData(rangeOverride);
  ...
  const handleDatePresetChange = async (preset) => {
    const range = dashboardData.applyDatePreset(preset);
    if (!range) {
      return;
    }
    const data = await dashboardData.refresh(range);
    if (data && data.length > 0) {
      dashboardCharts.updateChartData(data);
    }
    await loadUserData(range);
  };
  ...
  return (
    <div className='h-full'>
      <DashboardHeader
        ...
        datePreset={dashboardData.datePreset}
        datePresetOptions={dashboardData.datePresetOptions}
      ...
      <div className='mb-4'>
        ...
        >
          <ChartsPanel
            ...
            spec_token_model_line={dashboardCharts.spec_token_model_line}
