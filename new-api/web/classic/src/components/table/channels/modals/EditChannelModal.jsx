...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 57:
      return 'Enter Codex auth JSON with nested tokens';
    default:
      return '请输入渠道对应的鉴权密钥';
  }
}
        ...
        try {
          ...
          const hasRequiredTokens =
            ...
            ['id_token', 'access_token', 'refresh_token', 'account_id'].every(
              (field) => String(tokens[field] || '').trim() !== '',
            );
          if (!hasRequiredTokens) {
            showInfo(
              t(
                'Only JSON objects with tokens.id_token, tokens.access_token, tokens.refresh_token, and tokens.account_id are supported',
              ),
            );
            return;
          }
                        ...
                        {inputs.type === 57 ? (
                          <>
                            <Form.TextArea
                              ...
                              placeholder={t(
                                'Paste Codex auth JSON, for example:\n{\n  "tokens": {\n    "id_token": "...",\n    "access_token": "...",\n    "refresh_token": "...",\n    "account_id": "..."\n  }\n}',
                              )}
                              rules={
                                isEdit
                                  ? []
