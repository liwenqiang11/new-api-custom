...
func isNonEmptyJSONField(keyMap map[string]any, field string) bool {
	v, ok := keyMap[field]
	if !ok || v == nil {
		return false
	}
...
func validateCodexOAuthKey(raw string) error {
	...
	if _, err := codex.ParseOAuthKey(raw); err != nil {
		return err
	}
	return nil
}

func validateAntigravityOAuthKey(raw string) error {
	var keyMap map[string]any
	if err := common.Unmarshal([]byte(strings.TrimSpace(raw)), &keyMap); err != nil {
		return fmt.Errorf("Antigravity key must be a valid OAuth JSON object")
	}
	if isNonEmptyJSONField(keyMap, "access_token") {
		return nil
	}
	if token, ok := keyMap["token"].(map[string]any); ok && isNonEmptyJSONField(token, "access_token") {
		return nil
	}
	if tokenRaw, ok := keyMap["token"]; ok {
		tokenBytes, err := common.Marshal(tokenRaw)
		if err == nil {
			var tokenMap map[string]any
			if err := common.Unmarshal(tokenBytes, &tokenMap); err == nil && isNonEmptyJSONField(tokenMap, "access_token") {
				return nil
			}
		}
	}
	if !isNonEmptyJSONField(keyMap, "access_token") {
		return fmt.Errorf("Antigravity OAuth JSON must include access_token")
	}
	return nil
}

func getAntigravityOAuthKeys(keys string) ([]string, error) {
	...
	if strings.HasPrefix(trimmed, "[") {
		...
		if err := common.Unmarshal([]byte(trimmed), &arr); err != nil {
			return nil, fmt.Errorf("Antigravity batch keys must be OAuth JSON objects or a JSON array: %w", err)
		}
		cleanKeys := make([]string, 0, len(arr))
		for _, item := range arr {
			key := strings.TrimSpace(string(item))
			if key == "" {
				continue
			}
			if err := validateAntigravityOAuthKey(key); err != nil {
				return nil, err
			}
			cleanKeys = append(cleanKeys, key)
		}
		if len(cleanKeys) == 0 {
			return nil, fmt.Errorf("Antigravity OAuth JSON array cannot be empty")
		}
		return cleanKeys, nil
	}

	if err := validateAntigravityOAuthKey(trimmed); err == nil {
	...
	if len(cleanKeys) == 0 {
		return nil, fmt.Errorf("Antigravity OAuth JSON cannot be empty")
	}
	return cleanKeys, nil
}
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	if channel.Type == constant.ChannelTypeCodex {
		...
		if isAdd || trimmedKey != "" {
			if err := validateCodexOAuthKey(trimmedKey); err != nil {
				return err
			}
		}
	}
	...
	if channel.Type == constant.ChannelTypeAntigravity {
		trimmedKey := strings.TrimSpace(channel.Key)
		if isAdd || trimmedKey != "" {
			if _, err := getAntigravityOAuthKeys(trimmedKey); err != nil {
				return err
			}
		}
	}
...
func RefreshCodexChannelCredential(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"data": gin.H{
			...
			"account_id":   oauthKey.Tokens.AccountID,
...
func AddChannel(c *gin.Context) {
	...
	case "multi_to_single":
		...
		} else if addChannelRequest.Channel.Type == constant.ChannelTypeAntigravity {
			array, err := getAntigravityOAuthKeys(addChannelRequest.Channel.Key)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
	...
	case "batch":
		if addChannelRequest.Channel.Type == constant.ChannelTypeVertexAi && addChannelRequest.Channel.GetOtherSettings().VertexKeyType != dto.VertexKeyTypeAPIKey {
			...
			keys, err = getAntigravityOAuthKeys(addChannelRequest.Channel.Key)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
	...
	case "single":
		if addChannelRequest.Channel.Type == constant.ChannelTypeAntigravity {
			keys, err = getAntigravityOAuthKeys(addChannelRequest.Channel.Key)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
			...
			if len(keys) != 1 {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": "Antigravity single mode accepts exactly one OAuth JSON object",
				})
				return
			}
		} else {
...
func FetchModels(c *gin.Context) {
	...
	if req.Type == constant.ChannelTypeAntigravity {
		...
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": fmt.Sprintf("鑾峰彇Antigravity妯″瀷澶辫触: %s", err.Error()),
			})
			return
		}
