...
// tools[].type=image_generation: Responses API 图片生成工具，默认过滤以保留图片识别但避免误触发图片生成
func RemoveDisabledFields(jsonData []byte, channelOtherSettings dto.ChannelOtherSettings, channelPassThroughEnabled bool) ([]byte, error) {
	...
	// 默认移除 Responses API image_generation 工具。图片识别使用 input_image，
	// 不依赖该工具；过滤它可以避免 Codex 自动带工具时触发上游图片生成权限检查。
	if !channelOtherSettings.AllowResponsesImageGenerationTool {
		removeResponsesImageGenerationTool(data)
	}
...
func hasRemovableDisabledField(jsonData []byte, channelOtherSettings dto.ChannelOtherSettings) bool {
	...
	return (!channelOtherSettings.AllowServiceTier && values[0].Exists()) ||
		...
		(!channelOtherSettings.AllowResponsesImageGenerationTool && values[6].Exists())
}

func removeResponsesImageGenerationTool(data map[string]interface{}) {
	toolsAny, exists := data["tools"]
	if !exists {
		return
	}

	tools, ok := toolsAny.([]interface{})
	if !ok {
		return
	}
