...
// GeminiFlashImageHandler handles responses from flash/pro image models (e.g. gemini-3.1-flash-image)
// that use the generateContent endpoint and return images as InlineData parts.
func GeminiFlashImageHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// Flash image models: count generated images for quota, each ~1400 tokens
	imageCount := len(openAIResponse.Data)
	const flashImageTokens = 1400
	usage := &dto.Usage{
		PromptTokens:     flashImageTokens * imageCount,
		CompletionTokens: 0,
		TotalTokens:      flashImageTokens * imageCount,
	}
	if geminiResponse.UsageMetadata.TotalTokenCount != 0 {
		mapped := buildUsageFromGeminiMetadata(geminiResponse.UsageMetadata, info.GetEstimatePromptTokens())
		usage = &mapped
	}
