...
func OaiResponsesToChatStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	sendToolCallDelta := func(callID string, name string, argsDelta string) bool {
		...
		// Responses streams from Codex-style models may emit assistant text first
		// and then tool calls in the same turn. Do not drop those calls: Claude
		// clients rely on the final tool_use / tool_calls stop to continue.
		if !sendStartIfNeeded() {
			return false
		}
