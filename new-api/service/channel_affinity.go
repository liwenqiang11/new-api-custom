...
const (
	...
	ginKeyChannelAffinityDisabledFallback = "channel_affinity_disabled_fallback"
...
func ShouldSkipRetryAfterChannelAffinityFailure(c *gin.Context) bool {
	...
	// When the channel preferred by affinity is disabled and the
	// distributor already falls back to the normal selection path, the
	// affinity rule's skip_retry_on_failure flag must not block retries on
	// subsequent upstream errors, otherwise the request would just see the
	// same failure repeated on every retry attempt instead of trying the
	// other available channels.
	if v, ok := c.Get(ginKeyChannelAffinityDisabledFallback); ok {
		if b, ok := v.(bool); ok && b {
			return false
		}
	}
...
// MarkChannelAffinityDisabledFallback records that the channel preferred
// by affinity was disabled, and that the distributor fell back to the
// normal random selection path. Once this flag is set,
// ShouldSkipRetryAfterChannelAffinityFailure stops honoring the affinity
// rule's skip_retry_on_failure flag, so that subsequent upstream errors
// can still trigger a retry against the other available channels.
func MarkChannelAffinityDisabledFallback(c *gin.Context) {
	if c == nil {
		return
	}
	c.Set(ginKeyChannelAffinityDisabledFallback, true)
}

// IsChannelAffinityDisabledFallback reports whether the current request
// was routed via the normal selection path because the channel preferred
// by affinity was disabled.
func IsChannelAffinityDisabledFallback(c *gin.Context) bool {
	if c == nil {
		return false
	}
	v, ok := c.Get(ginKeyChannelAffinityDisabledFallback)
	if !ok {
		return false
	}
	b, ok := v.(bool)
