...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	requestedModel := strings.TrimSpace(ctx.GetString("antigravity_model_requested"))
	if requestedModel == "" {
		requestedModel = strings.TrimSpace(relayInfo.OriginModelName)
	}
	upstreamModel := strings.TrimSpace(ctx.GetString("antigravity_model_upstream"))
	if upstreamModel == "" {
	...
	if upstreamModel != "" && requestedModel != "" && !strings.EqualFold(upstreamModel, requestedModel) {
		other["is_model_mapped"] = true
	}
	if mappedModel := strings.TrimSpace(ctx.GetString("antigravity_model_mapped")); mappedModel != "" {
		other["mapped_model_name"] = mappedModel
	}
	if redirectReason := strings.TrimSpace(ctx.GetString("antigravity_model_redirect_reason")); redirectReason != "" {
		other["model_redirect_reason"] = redirectReason
	}
