...
func runCodexCredentialAutoRefreshOnce() {
	...
	for {
		...
		for _, ch := range channels {
			...
			refreshToken := strings.TrimSpace(oauthKey.Tokens.RefreshToken)
			if refreshToken == "" {
				continue
			}

			expiredAt, ok := ExtractExpiryFromJWT(oauthKey.Tokens.AccessToken)
			if ok && expiredAt.Sub(now) > codexCredentialRefreshThreshold {
				continue
			}
