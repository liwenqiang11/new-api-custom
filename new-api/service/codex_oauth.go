...
func exchangeCodexAuthorizationCode(
	...
	if strings.TrimSpace(payload.IDToken) == "" || strings.TrimSpace(payload.AccessToken) == "" || strings.TrimSpace(payload.RefreshToken) == "" || payload.ExpiresIn <= 0 {
		return nil, errors.New("codex oauth token response missing id_token, access_token, refresh_token, or expires_in")
	}
	return &CodexOAuthTokenResult{
		IDToken:      strings.TrimSpace(payload.IDToken),
		AccessToken:  strings.TrimSpace(payload.AccessToken),
...
func ExtractExpiryFromJWT(token string) (time.Time, bool) {
	claims, ok := decodeJWTClaims(token)
	if !ok {
